...
while(rerun){
p0A = rbeta(4, 1.67, 0.4)
p0B = rbeta(4, 1.67, 0.4)
muA = rchisq(4,df=10)
muB = rchisq(4,df=10)
omuA = muA*(1-p0A)
omuB = muB*(1-p0B)
...
f_mod <- function() {
	for(i in 1:N){
		Y[i] ~ dpois(lambda[i])
		lambda[i] <- T[i]*(0.000001) + (1-T[i])*mu[i]
		log(mu[i]) <- B[GRP[i]]
		T[i] ~ dbern(p[i])
		logit(p[i]) <- BP[GRP[i]]
	}
	...
	logit(E_P0_Oral[1]) <- BP[1]
	logit(E_P0_Oral[2]) <- BP[2]
	logit(E_P0_Oral[3]) <- BP[3]
	logit(E_P0_Oral[4]) <- BP[4]

	logit(E_P0_Panc[1]) <- BP[5]
	logit(E_P0_Panc[2]) <- BP[6]
	logit(E_P0_Panc[3]) <- BP[7]
	logit(E_P0_Panc[4]) <- BP[8]
}
...
inivals <- list( list(  B=rep(0,8), BP=rep(0,8) ) )

fit <- bugs(
	dats, inits=inivals, parameters.to.save = c("E_MU_Oral","E_MU_Panc","E_P0_Oral","E_P0_Panc"), 
...
#par(mfrow=c(1,2))
#plot(c(p0A,p0B),predP0); lines(c(-9999,9999),c(-9999,9999),col="red")
#plot(c(muA,muB),predMU); lines(c(-9999,9999),c(-9999,9999),col="red")

M_Oral = fit$sims.list[["E_MU_Oral"]]
M_Panc = fit$sims.list[["E_MU_Panc"]]
P_Oral = fit$sims.list[["E_P0_Oral"]]
P_Panc = fit$sims.list[["E_P0_Panc"]]
...
sim = list(QMP=QMP, QMS=QMS, MU.TRUE=true.MU, MU.PRED=pred.MU, P0.TRUE=true.P0, P0.PRED=pred.P0)

outpath = paste0("sim_ZIP_omu10_gn_","n",each_num,"_T",TCONC*100,"_delta",delta*10,"_grps4_spct0_pe.Rdata")
