...
########################################################################################################################################################################################################
#######
#######    mouth_saliva vs other_mouth (4 ICDC)
#######
########################################################################################################################################################################################################
########################################################################################################################################################################################################
########################################################################################################################################################################################################
...
for(i in 2:length(all.cnt[1,])){
	libsizes = c(libsizes, sum(all.cnt[,i]))
    all.cnt[,i] = all.cnt[,i]/sum(all.cnt[,i])
}
libtab = data.frame( SampleID=colnames(all.cnt)[-1], LIB=libsizes, LOGLIB=log(libsizes) )
all.met = merge(all.met, libtab, by="SampleID")
...
inclusionSites = c()
inclusionSites = c( inclusionSites, c("mouth_buccal","mouth_gum","mouth_saliva","mouth_tongue") )

all.met = all.met[all.met$SITE %in% inclusionSites,]
all.met = all.met[all.met$STUDY != "NDRI",]



consistentSamples = unique(
    intersect(colnames(all.cnt), all.met$SampleID)
)
all.met = all.met[all.met$SampleID %in% consistentSamples,]
all.cnt = all.cnt[,c("OTU.ID",colnames(all.cnt)[colnames(all.cnt) %in% consistentSamples])]
sub.cnt = all.cnt
...
mouth_sites = c( "mouth_tongue" )
mouth_short = c( "tongue" )

qqqq = 1
for(qqqq in 1:length(mouth_sites)){
sub.met = all.met[all.met$SITE %in% c("mouth_saliva",mouth_sites[qqqq]),]
sub.met = sub.met[!is.na(sub.met$GENDER),]
sub.met$GUT = 1.0*(sub.met$SITE == "mouth_saliva")
...
for(i in 1:length(sub.cnt[,1])){
    ...
    testSet2 = testSet[,c("SampleID","Y","PRE_GRP","GUT","subject_id", "LOGLIB")]
...
f_mod <- function() {
    for(i in 1:N){
        Y[i] ~ dbin(p[i], 1)
        logit(p[i]) <- B[GRP[i]] + L*LIB[i] + U[SBJ[i]]
    }
...
dats = list(
	...
	LIB = (btab$LOGLIB-mean((btab$LOGLIB)))/sd(btab$LOGLIB),
	M = length(unique(btab$b_subject_grp)),
    N = length(btab$Y)
)

inivals <- list( list(  B=rep(0,8), tau=2, L=0 ) )
...
outpath = paste0("...\\GENUS_saliva_vs_",mouth_short[qqqq],"_ICDC_libadjust_random_effect.csv")
...
########################################################################################################################################################################################################
#######
#######    mouth_gum vs other_mouth (4 ICDC)
#######
########################################################################################################################################################################################################
########################################################################################################################################################################################################
########################################################################################################################################################################################################
...
mouth_sites = c( "mouth_saliva","mouth_tongue" )
mouth_short = c( "saliva","tongue" )

qqqq = 1
for(qqqq in 1:length(mouth_sites)){
sub.met = all.met[all.met$SITE %in% c("mouth_gum",mouth_sites[qqqq]),]
sub.met = sub.met[!is.na(sub.met$GENDER),]
sub.met$GUT = 1.0*(sub.met$SITE == "mouth_gum")
sub.met$PRE_GRP = sub.met$ICDC
...
outpath = paste0("...\\GENUS_gum_vs_",mouth_short[qqqq],"_ICDC_libadjust_random_effect.csv")
write.csv(results, file=outpath, row.names=FALSE)
...
########################################################################################################################################################################################################
#######
#######    mouth_buccal vs other_mouth (4 ICDC)
#######
########################################################################################################################################################################################################
########################################################################################################################################################################################################
########################################################################################################################################################################################################
...
mouth_sites = c( "mouth_gum","mouth_saliva","mouth_tongue" )
mouth_short = c( "gum","saliva","tongue" )

qqqq = 1
for(qqqq in 1:length(mouth_sites)){
sub.met = all.met[all.met$SITE %in% c("mouth_buccal",mouth_sites[qqqq]),]
sub.met = sub.met[!is.na(sub.met$GENDER),]
sub.met$GUT = 1.0*(sub.met$SITE == "mouth_buccal")
sub.met$PRE_GRP = sub.met$ICDC
...
outpath = paste0("...\\GENUS_buccal_vs_",mouth_short[qqqq],"_ICDC_libadjust_random_effect.csv")
write.csv(results, file=outpath, row.names=FALSE)
...
all.met[all.met$SITE %in% mouth_sites,]$SITEV2 = "mouth"
...
qqqq = 1
sub.met = all.met[all.met$SITEV2 %in% c("gut","mouth"),]
sub.met = sub.met[!is.na(sub.met$GENDER),]
...
outpath = paste0("...\\GENUS_gut_vs_mouth_ICDC_libadjust_random_effect.csv")
write.csv(results, file=outpath, row.names=FALSE)
...
qqqq = 1
for(qqqq in 1:length(mouth_sites)){
sub.met = all.met[all.met$SITEV2 %in% c("gut",mouth_sites[qqqq]),]
sub.met = sub.met[!is.na(sub.met$GENDER),]
sub.met$GUT = 1.0*(sub.met$SITEV2 == "gut")
sub.met$PRE_GRP = sub.met$ICDC
...
outpath = paste0("...\\GENUS_gut_vs_",mouth_short[qqqq],"_ICDC_libadjust_random_effect.csv")
write.csv(results, file=outpath, row.names=FALSE)
